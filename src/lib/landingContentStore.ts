...
export const LANDING_CONTENT_ID = 1;

export const DEFAULT_LANDING_CONTENT: Record<string, string> = {
...
export function normalizeLandingContent(raw: unknown): Record<string, string> {
  if (!raw || typeof raw !== "object") return { ...DEFAULT_LANDING_CONTENT };
  const rec = raw as Record<string, unknown>;
  const out: Record<string, string> = { ...DEFAULT_LANDING_CONTENT };
...
export async function ensureLandingContentRow(pool: Pool) {
  await pool.query(
    `INSERT INTO public.landing_content (id, content)
     VALUES ($1, $2::jsonb)
     ON CONFLICT (id) DO NOTHING`,
    [LANDING_CONTENT_ID, JSON.stringify(DEFAULT_LANDING_CONTENT)]
  );
}

export async function loadLandingContentRow(pool: Pool) {
